...
class ManagerMailing(StatesGroup):
    SendMessage = State()


@dp.callback_query_handler(text=ManagerCallbacks.create_mailing.value)
async def create_mailing(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text(await messages.get_message("mailing_start"),
                                     reply_markup=None)
    await ManagerMailing.SendMessage.set()


@dp.message_handler(state=ManagerMailing.SendMessage, content_types=types.ContentType.ANY)

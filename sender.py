...
async def sender_from_db():
    ...
    for item in range(postcard_count):
        ...
        if user_who_send is not None:
            user_who_sent_name = user_who_send.full_name()
            if user_to_send['telegram_id'] is not None:
                try:
                    await bot.send_photo(
                        user_to_send['telegram_id'],
                        postcard['file_id'],
                        caption=f"{user_who_sent_name} отправляет вам благодарность"
                    )
            ...
            if user_to_send['email'] is not None:
                try:
                    await send_email_photo(
                        ...
                        f"{user_who_sent_name} отправляет вам благодарность.\n"
            ...
            if user_to_send['telegram_id'] is not None:
                try:
                    ...
                    log(INFO, f"Failed send message [{user_to_send['telegram_id']}]")
            if user_to_send['email'] is not None:
                try:
                    await send_email_photo(
                        user_to_send['email'],
